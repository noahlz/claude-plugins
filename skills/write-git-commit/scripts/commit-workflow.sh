...
action_prepare() {
  ...
  COST_DELTA_OUTPUT=$(bash "${CLAUDE_PLUGIN_ROOT}/skills/write-git-commit/scripts/claude-cost-delta.sh" "$METRICS_FILE" 2>&1)
  COST_DELTA_EXIT=$?
...
action_append_metrics() {
  ...
  # Append metrics
  if bash "${CLAUDE_PLUGIN_ROOT}/skills/write-git-commit/scripts/append-cost-metrics.sh" \
