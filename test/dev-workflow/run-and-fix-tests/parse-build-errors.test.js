...
`;
  ...
  it('limits errors to 30 maximum', () => {
    const config = {
      build: {
        logFile: 'build.log',
        errorPattern: 'ERROR:'
  ...
  it('sets truncated flag when > 30 errors', () => {
    const config = {
      build: {
        logFile: 'build.log',
        errorPattern: 'ERROR:'
    ...
    // Generate 35 errors
    const errors = Array.from({ length: 35 }, (_, i) => `ERROR: Error ${i + 1}`);
    const logContent = errors.join('\n');
    ...
    assert.equal(result.errors.length, 30, 'Should limit to 30 errors');
    assert.equal(result.totalErrors, 35, 'Should count all 35 errors');
