#!/bin/bash
# Tests for skills/write-git-commit/scripts/claude-cost-delta.sh
...
run_delta_script() {
  ...
  "$CLAUDE_PLUGIN_ROOT/skills/write-git-commit/scripts/claude-cost-delta.sh" "$metrics_file" 2>/dev/null
}
...
run_delta_script_with_stderr() {
  ...
  "$CLAUDE_PLUGIN_ROOT/skills/write-git-commit/scripts/claude-cost-delta.sh" "$metrics_file" 2>&1
}
...
run_delta_script_exit_code() {
  ...
  "$CLAUDE_PLUGIN_ROOT/skills/write-git-commit/scripts/claude-cost-delta.sh" "$metrics_file" > /dev/null 2>&1
  echo $?
}
...
EOF
  ...
  local output=$(bash "$CLAUDE_PLUGIN_ROOT/skills/write-git-commit/scripts/claude-cost-delta.sh" ".claude/cost-metrics.jsonl" 2>&1)
